...
def rawdata_duckdb_sql(data: str = "train") -> str:
    return (" WITH h as ( "
            " SELECT "
            "   date,"
            "   locale,"
            "   locale_name, "
            "   SUM(CASE WHEN type == 'Holiday' THEN 1 ELSE 0 END)::BIGINT as calendar_holiday,"
            "   SUM(CASE WHEN type == 'Event' THEN 1 ELSE 0 END)::BIGINT as calendar_event,"
            "   SUM(CASE WHEN type == 'Additional' THEN 1 ELSE 0 END)::BIGINT as calendar_additional,"
            "   SUM(CASE WHEN type == 'Transfer' THEN 1 ELSE 0 END)::BIGINT as calendar_transfer,"
            "   SUM(CASE WHEN type == 'Bridge' THEN 1 ELSE 0 END)::BIGINT as calendar_bridge,"
            "   SUM(CASE WHEN transferred == true THEN 1 ELSE 0 END)::BIGINT as calendar_transferred,"
            "   count(*)::BIGINT  AS num_holiday "
            ...
            "   (CASE WHEN (last_day(t.date)==t.date OR datepart('day', t.date)==15) then 1 else 0 END)::BIGINT AS payment_wages,"
            "   t.store_nbr, "
            "   t.family, "
            "   %s "
            "   t.onpromotion, "
            ...
            "   o.dcoilwtico, "
            "   tx.transactions, "
            "   COALESCE(hl.calendar_holiday,hr.calendar_holiday,hn.calendar_holiday, 0)::BIGINT as calendar_holiday,"
            "   COALESCE(hl.calendar_event,hr.calendar_event,hn.calendar_event, 0)::BIGINT as calendar_event,"
            "   COALESCE(hl.calendar_additional,hr.calendar_additional,hn.calendar_additional, 0)::BIGINT as calendar_additional,"
            "   COALESCE(hl.calendar_transfer,hr.calendar_transfer,hn.calendar_transfer, 0)::BIGINT as calendar_transfer,"
            "   COALESCE(hl.calendar_bridge,hr.calendar_bridge,hn.calendar_bridge, 0)::BIGINT as calendar_bridge,"
            "   COALESCE(hl.calendar_transferred,hr.calendar_transferred,hn.calendar_transferred, 0)::BIGINT as calendar_transferred \n"
            ...
            % (" t.sales, " if data == "train" else " (0)::BIGINT as sales, ", data)
            )
...
def rawdata(data: str = "train") -> duckdb.DuckDBPyRelation:
    ...
    os.chdir(root_dir)
    _data = duckdb.sql(rawdata_duckdb_sql(data=data))
